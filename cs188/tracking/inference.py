...
import busters
import game
from pygame._numpysndarray import samples
from keyring.util.escape import LEGAL_CHARS
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            weights = [emissionModel[util.manhattanDistance(self.particles[i], pacmanPosition)] for i in  range(self.numParticles)]
            if sum(weights) == 0:
                self.initializeUniformly(gameState)
            else:
                samples = util.nSample(weights, self.particles, self.numParticles)
...
#         util.raiseNotDefined()
    ...
    def elapseTime(self, gameState):
        ...
        for i in range(self.numParticles):
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldParticles[i]))
...
#         util.raiseNotDefined()
    ...
    def getBeliefDistribution(self):
        ...
        possPos =  self.legalPositions 
        possPos.append(self.getJailPosition())
        for pos in possPos:
            beliefs[pos] = float(self.particles.count(pos))/self.numParticles
        return beliefs
#         util.raiseNotDefined()
...
#         print [self.legalPositions[i%len(self.legalPositions)] for i in range(self.numParticles)]
        self.posPart = list(itertools.product(self.legalPositions,repeat=self.numGhosts))
        self.particles = [self.posPart[i%len(self.posPart)] for i in range(self.numParticles)]
#         
...
#         print emissionModels[0][util.manhattanDistance(pacmanPosition, self.getJailPosition(0))]
#         print noisyDistances
        
        weights = [1.0]*self.numParticles
        samples = self.particles
        for i in range(self.numGhosts):
            if noisyDistances[i] == None:
                samples = [self.getParticleWithGhostInJail(p, i) for p in samples]
            else:
                for j in range(self.numParticles):
                    weights[j] *= emissionModels[i][util.manhattanDistance(pacmanPosition, samples[j][i])]
        if sum(weights) == 0:
            self.initializeParticles()
        else:
            self.particles = util.nSample(weights, samples, self.numParticles)
#         util.raiseNotDefined()
    ...
    def getBeliefDistribution(self):
        ...
        possPos = list(itertools.product(self.legalPositions+jailPositions,repeat=self.numGhosts))
        counter = Counter(self.particles)
        for pos in possPos:
            beliefs[pos] = float(counter[pos])/self.numParticles
        return beliefs
#         util.raiseNotDefined()
...
def setGhostPositions(gameState, 
                      ghostPositions):
