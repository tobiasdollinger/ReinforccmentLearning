...
def enhancedFeatureExtractorDigit(datum):
    ...
    countBlack = 0.0
    countWhite = 0.0
    listBlackX = [None]*28*28
    listBlackY = [None]*28*28
    listWhite = [None]*28*28
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if datum.getPixel(x, y) > 0:
                listBlackX[int(countBlack)] = x
                listBlackY[int(countBlack)] = y
                countBlack +=1.0
            else:
                listWhite[int(countWhite)] = (x,y)
                countWhite += 1.0
    
    "'''Detect number of separated white regions'''"
    def getNeighboringWhites(point):
        listWhiteNeigh=[]
        for i in [-1,1]:
            try:
                if not point[0]+i < 0:
                    if datum.getPixel(point[0]+i, point[1]) == 0:
                        listWhiteNeigh.append((point[0]+i, point[1]))
            except IndexError:
                pass
            try:
                if not point[1]+i < 0:
                    if datum.getPixel(point[0], point[1]+i) == 0:
                        listWhiteNeigh.append((point[0], point[1]+i))
            except IndexError:
                pass
        return listWhiteNeigh
        
    def getWhiteRegionFromTuple(startPoint):
        listConnectedWhite = [startPoint]
        idx = 0
        while True:
            newList = getNeighboringWhites(listConnectedWhite[idx])
            listConnectedWhite += [xy for xy in newList if xy not in listConnectedWhite]
            idx += 1
            if idx == len(listConnectedWhite):
                break
        return listConnectedWhite
        
    counterWhiteAreas = 0
    listWhite = listWhite[:int(countWhite)]
    while len(listWhite) > 0:
        counterWhiteAreas += 1
        listConnectedWhites = getWhiteRegionFromTuple(listWhite[0])
        listWhite = [xy for xy in listWhite if xy not in listConnectedWhites]

    if counterWhiteAreas == 1:
        features[(2)] = 1
        features[(3)] = 0
        features[(4)] = 0
        features[(5)] = 0
    elif counterWhiteAreas == 2:
        features[(2)] = 0
        features[(3)] = 1
        features[(4)] = 0
        features[(5)] = 0
    elif counterWhiteAreas == 3:
        features[(2)] = 0
        features[(3)] = 0
        features[(4)] = 1
        features[(5)] = 0
    ...
    listBlackX = listBlackX[:int(countBlack)]
    listBlackY = listBlackY[:int(countBlack)]
    arrayBlackX = np.asarray(listBlackX)
    arrayBlackY = np.asarray(listBlackY)
    medianX = np.median(arrayBlackX)
    medianY = np.median(arrayBlackY)
    meanXBlack = np.mean(arrayBlackX)
    meanYBlack = np.mean(arrayBlackY)
    if medianX < meanXBlack:
        features[(0)] = 0
    else:
        features[(0)] = 1
    if medianY < meanYBlack:
        features[(1)] = 0
    else:
        features[(1)] = 1
...
def enhancedPacmanFeatures(state, action):
    ...
    "*** YOUR CODE HERE ***"
    successor = state.generateSuccessor(0, action)
    numberGhost = successor.getNumAgents()-1
    pacmanPosition = successor.getPacmanPosition()
    foodCount = successor.getFood().count()
    capsuleCount = len(successor.getCapsules())
    try:
        minDistantFromFood = min([util.manhattanDistance(pacmanPosition, xy2) for xy2 in successor.getFood().asList()])
    except ValueError:
        minDistantFromFood = 0
    try:
        minDistantFromCapsule = min([util.manhattanDistance(pacmanPosition, xy2) for xy2 in successor.getCapsules()])
    except ValueError:
        minDistantFromCapsule = 0
    scaredTimerGhosts =  [successor.getGhostState(i).scaredTimer for i in range(1,numberGhost+1)]
    try:
        minDistantFromNormalGhost = min([util.manhattanDistance(pacmanPosition, successor.getGhostPosition(i)) for i in range(1,numberGhost+1) if scaredTimerGhosts[i-1] == 0])
    except ValueError:
        minDistantFromNormalGhost = 0
    try:
        minDistantFromScaredGhost = min([util.manhattanDistance(pacmanPosition, successor.getGhostPosition(i)) for i in range(1,numberGhost+1) if scaredTimerGhosts[i-1] != 0])
    except ValueError:
        minDistantFromScaredGhost = 0
        
    features['foodCount'] = foodCount
    features['capsuleCount'] = capsuleCount
    features['minDistantFromFood'] = minDistantFromFood
    features['minDistantFromCapsule'] = minDistantFromCapsule
    features['minDistantFromNormalGhost'] = minDistantFromNormalGhost
    features['minDistantFromScaredGhost'] = minDistantFromScaredGhost
